...
**Decisión de Luis (5/Ago, RATIFICADA 6/Ago): NO arreglar** (el mapeo de negocio correcto —qué `transportMode`/oficinas/totals/owner debe llevar un registro creado por lote— es una decisión de producto que no se puede verificar automáticamente). Al revisar los 5 puntos pendientes bajo el objetivo "corregir todos" (6/Ago), Luis confirmó dejar este aparcado: es el único de los 5 que exige una decisión de producto y no una corrección verificable. Los tests (`tests/services/batchProcessor.test.js`) **fijan el comportamiento real**: validación OK, procesamiento → todo item `failed`. Si algún día se cablea el batch a producción, esos tests obligarán a revisar los `create*`. Salió al cubrir un módulo al 0% contra Mongo real, sin mockear los modelos.
...
### `certificateService.importCertificate`: al rechazar un cert leía `validation.error` (inexistente) → el motivo del rechazo se perdía (CORREGIDO 6 de agosto de 2026)

Cuando `_validateCertificate` rechaza un certificado (emisor no FNMT, sin permiso de firma, expirado…) devuelve `{valid:false, errors:[...], warnings}` — el detalle va en `errors` (array), **no** en un campo `error`. Pero `importCertificate` construía la respuesta de fallo con `error: validation.error` (línea 108), que es `undefined`. Consecuencia: el cliente que solo mirase el campo `error` de la respuesta no veía **por qué** se rechazó el certificado (recibía `success:false` con `error: undefined`). No era un fallo de seguridad ni bloqueaba el rechazo —el certificado inválido SÍ se rechaza y no se almacena— y el motivo real SÍ llegaba por `luciAnalysis.summary`, de donde lo lee la UI.

**Corregido (6 de agosto de 2026).** `importCertificate` ahora expone el motivo en `error` (string, `validation.errors.join('; ')`, con fallback `"Certificado inválido"`) y añade `errors` (el array crudo), sin tocar `luciAnalysis` — el cambio es aditivo, no rompe a los consumidores que ya leen `luciAnalysis.summary`. Tests (`tests/services/certificateService.test.js`) actualizados: verifican que `error`/`errors` traen el motivo real (emisor no FNMT, sin permiso de firma). Discriminancia verificada: con el fix del `src` en stash, el test de emisor no FNMT falla (`r.error` es `undefined`). 27/27 pass. Cobertura del módulo generando P12 reales con node-forge contra el parseo y cifrado reales, sin mockear nada del código bajo prueba.
...
- **Doble definicion de `exports.amend` → primera version codigo muerto (CORREGIDO 6 de agosto de 2026).** `ensController.js` definia `exports.amend` dos veces: la primera (~L313) delegaba en `ensService.amendDeclaration`; la segunda (~L835) hace la enmienda IE313 real via `aeatSubmitService` **y ademas aplica `ensureSameTenant`**. La segunda sobrescribia a la primera en tiempo de carga del modulo, de modo que el router (`/:id/amend`) siempre montaba la IE313 y la primera version era codigo inalcanzable — trampa de mantenimiento (un cambio sobre la "primera" `amend` no tendria efecto). **Corregido:** eliminada la version muerta (la primera). Se conserva la IE313 por ser la funcionalmente completa y la unica que aisla por tenant; ya la ejercitan los tests `tests/controllers/ensController.db.test.js` (`describe('amend (IE313)')`, 48/48 pass). El comportamiento observable no cambia (la ruta ya montaba esta version).

- **Inconsistencia de enums en subdocumentos `documents` (RESUELTO — divergencia intencionada, documentada 6 de agosto de 2026).** El `documents` raiz de `ENSDeclaration` usa el enum `['CMR','BL','AWB','INVOICE','PACKING_LIST','CERTIFICATE','OTHER']`, mientras que los `documents` **por item** (`GoodsItemSchema`) usan codigos UCC `['N380','N714','N722','N730','N785','OTHER']`. Tras revisarlo **no es un error a unificar**: son dos conceptos distintos. El `documents` por-item son documentos **declarados a AEAT** (codigos UCC del anexo B, los mismos que consumen los builders XML `h1XmlBuilder`/`h7XmlBuilder`/`aeatSubmitService`); el `documents` raiz son **ficheros adjuntos operativos** que el usuario sube en la app (llevan `url`/`name`/`documentNumber`, los recibe `ensController` en `POST /:id/documents`) y no se envian a AEAT, por lo que usar etiquetas legibles es lo correcto para la UI. Unificarlos romperia o el mapeo AEAT o la UX de adjuntos. **Corregido documentando la divergencia intencionada** con comentarios en ambos subdocumentos de `src/models/ENSDeclaration.js`.
...
### `oeaService.getById/getByEORI/getByNIF`: getters de OEA sin filtro de propiedad → fuga cross-tenant de NIF/EORI/representante legal (6 de agosto de 2026)

El servicio OEA (`src/services/oeaService.js`) protege la mayoría de sus operaciones con un helper `_loadOwnedOEA(id, userId)` que rechaza (lanza "no encontrada") si el `userId` no es el `createdBy` de la certificación. Pero **tres getters de solo lectura no pasaban por ese helper**: `getById` hacía `OEA.findById(id)`, `getByEORI` hacía `OEA.findByEORI(eori)` y `getByNIF` hacía `OEA.findOne({ 'organization.nif': nif })`, **sin ninguna comprobación de propiedad**. Los tres controladores (`oeaController.getById/getByEORI/getByNIF`) y sus rutas (`routes/oea.js:50/53/56`) solo montan `auth`, sin guard de tenant, delegando el aislamiento en el service.

**Impacto (fuga cross-tenant, RGPD):** cualquier usuario autenticado que conociera (o enumerara) el `_id`, el EORI o el NIF de una certificación OEA de **otra** organización recibía HTTP 200 con el documento completo: razón social, **NIF y EORI**, tipo y estado de certificación, y datos del representante legal — información fiscal identificativa de terceros clientes, expuesta por manipulación de URL. Los endpoints por EORI y por NIF son especialmente sensibles porque el propio parámetro de búsqueda es un dato RGPD y permiten confirmar la existencia de una certificación a partir de un identificador fiscal. Misma familia que la fuga de `getAiDeclarationAnalysis` (`7a7e255`): un mecanismo de aislamiento presente en el resto del service pero ausente justo en estos getters.

**Corregido** (`fix:` los tres métodos aceptan ahora `userId = null` y devuelven `null` —el controller lo traduce a 404— si `userId && oea.createdBy && String(oea.createdBy) !== String(userId)`; se respeta la OEA legada sin `createdBy` y las llamadas internas sin `userId`, igual que hace `_loadOwnedOEA`). Los tres controladores pasan `req.user?._id || req.user?.id`. Fijado con regresión en `tests/controllers/oeaController.branches.db.test.js` (tests `'should prevent user B from accessing user A OEA by ID/EORI/NIF'`: siembran una OEA de la organización A y verifican HTTP 404 al consultarla con un usuario de la organización B). Prueba de discriminancia verificada con `git stash` del src (service + controller): sin el fix los tres tests FALLAN (devuelven 200 con los datos). Salió al ampliar la cobertura de ramas de `oeaController`, sin mockear el service.

### `apiKeyAuth`: la API key entrante se filtraba en los logs en cada intento inválido (6 de agosto de 2026)

El middleware de autenticación por API key (`src/middleware/apiKeyAuth.js`) rechaza las peticiones cuya key no existe en BD. En ese camino de rechazo (L33) registraba `logger.warn(\`Invalid API key attempt: ${apiKey.substring(0, 12)}...\`)`, es decir, **volcaba los primeros 12 caracteres de la key recibida al log**. Sólo 11 líneas más abajo (L44), para una key **válida** bloqueada por IP, el mismo middleware ya loguea correctamente `keyDoc.keyPrefix` (el prefijo público almacenado en BD, no sensible) — de modo que el patrón seguro ya existía en el propio fichero y L33 era una inconsistencia.

**Impacto (fuga de secreto en logs):** el prefijo `lca_` es público (es el `keyPrefix`), pero `substring(0, 12)` incluye además varios caracteres del cuerpo secreto de la key. Un intento con una key válida enviada por el header equivocado, un typo cerca del prefijo o un log agregado a un sistema de terceros exponía material de una credencial de API en texto plano. Misma familia que la fuga de `auditService.scrub` (apiKey nunca redactada): un secreto que acaba en reposo en los logs. Clasificación MEDIUM.

**Corregido** (`fix:` L33 pasa a `logger.warn(\`Invalid API key attempt (length: ${apiKey.length})\`)`; se registra sólo la longitud para diagnóstico, sin ningún fragmento de la key). Fijado con regresión en `tests/middleware/apiKeyAuth.branches.test.js` (test `'SECURITY (regresión fix 6/Ago): la API key inválida NO se filtra en el log'`: espía `logger.warn` y verifica que ningún argumento contiene la key entrante, su prefijo de 12 caracteres ni el marcador secreto). Discriminancia verificada con `git stash` del src: sin el fix el test FALLA (la key aparece en el mensaje). Salió al cubrir la cobertura de ramas de `apiKeyAuth` (0%→90%B), sin mockear el middleware.

**Hallazgos NO corregidos (observaciones, cambios de arquitectura fuera del alcance del mandato):**
- El `rateLimitStore` es un `Map` en memoria (L11). En despliegue multi-proceso (pm2 cluster) cada proceso tiene su propio contador → un atacante podría repartir peticiones entre procesos y diluir el rate limit. Ya hay un comentario en el código apuntando a migrar a Redis. Operacional MEDIUM; requiere infraestructura, no se toca aquí.
- La comparación del hash SHA-256 de la key la resuelve MongoDB con `findOne` (no en tiempo constante). Con un hash de 256 bits el timing attack es poco práctico. LOW; a nivel de BD.
